...
class CardHighlightOverlay(QWidget):
    ...
    # ------------------------------------------------------------------
    # Global mouse hook — detect clicks on highlighted cards
    # ------------------------------------------------------------------
    ...
    def _on_mouse_click(self, x: int, y: int) -> None:
        ...
        for region_idx, talent_name in self._slot_hits:
            ...
            bx = r["x"] - 4
            by = r["y"] - self._highlight_above
            bw = r["w"] + 8
            bh = r["h"] + self._highlight_above + self._highlight_below
    ...
    def paintEvent(self, event) -> None:
        # Nothing to draw
        if not self._slot_hits or not self._missing:
            return
